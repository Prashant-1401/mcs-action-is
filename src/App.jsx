...
const SHEET_SCRIPT_URL = import.meta.env.VITE_SHEET_SCRIPT_URL || "https://script.google.com/macros/s/AKfycbwTMseU2yi5M5lnqS5ke1zyv2oAhmxp_sowssK-0-iDUX5PTbo8-d91A69IQJLI7vqhVw/exec";
const _DEFAULT_SHEET_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwTMseU2yi5M5lnqS5ke1zyv2oAhmxp_sowssK-0-iDUX5PTbo8-d91A69IQJLI7vqhVw/exec";
...
function Shell({ children, page, setPage, user, onLogout, onQuickAdd, pendingCount, auditCount, activeMtg, onResumeActiveMtg, mtgRunning, mtgElapsed, notifications, onMarkAllRead, unreadCount, users, actions, onShowSupport, onShowProfile, onShowAdminNotifs }) {
  ...
  const canAccessPage = (id) => {
    ...
    if (id === 0 || id === 1 || id === 4) return true;
    if (id === 2) return userPerms.canEditActions;
    if (id === 3) return userPerms.canViewDashboard;
    if (id === 99) return user?.masterAccess === true || user?.masterAccess === "true";
    return false;
  };
    ...
    <div style={{
      ...
      {["Escalated Action", ...STATUS_LIST].map(col => {
        ...
        if (isEscalatedCol) {
          ...
          colCards = fa.filter(a => a.status === col);
        }
...
function EscMatrixTab({ escMatrix, setEscMatrix, onSave, isAdmin, canModify }) {
  const PRIORITIES = ["CRITICAL", "WARNING", "NORMAL"];
  const ROLES = ["Operator", "Supervisor", "Shift Engineer", "HOD", "Plant Head", "MD", "Admin"];
  const TARGETS = ["Supervisor", "Shift Engineer", "HOD", "Plant Head", "MD", "Admin", "All"];
...
function EscalationsPage({ actions, setActions, audit, users, escMatrix, plants, depts, user }) {
  ...
  const getActionEscalationState = (action, matrix) => {
    ...
    // Resolve responsible user's role
    const respUser = (users || []).find(u => u.name === action.responsible);
    const respRole = respUser?.role || action.responsibleRole || "";

    // Find ALL applicable tiers (matching fromRole) whose threshold is met —
    // the highest level reached is the "current" escalation state
    ...
    // The highest level tier reached = current escalation state
    const matchedTier = matched[matched.length - 1];

    return {
      tier: matchedTier,
      allMatchedTiers: matched,    // full chain of levels triggered so far
      hrsOverdue,
...
function MasterPage({ user, plants, setPlants, depts, setDepts, users, setUsers, escMatrix, setEscMatrix, mtgPresets, setMtgPresets, machines, setMachines, refreshMaster, lastWriteRef, roles = [], setRoles }) {
  const isAdmin = user?.role === "Admin";
  ...
  const TABS = [["users", "👥 Users"], ["plants", "🏭 Plants"], ["depts", "🗂 Depts"], ["machines", "⚙ Machines"], ["roles", "🎭 Roles"], ["org", "🌳 Org"], ["escmatrix", "🚨 Escalation"], ["presets", "🎙 Presets"]];
  ...
  return (
    <div className="fade-in">
      ...
      {/* ── ESCALATION MATRIX ── */}
      {tab === "escmatrix" && <EscMatrixTab escMatrix={escMatrix} setEscMatrix={setEscMatrix} onSave={isAdmin || user?.masterAccess ? () => saveToSheet("EscalationMatrix", escMatrix || []) : null} isAdmin={isAdmin} canModify={canModify} />}
