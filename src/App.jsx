...
/* ===================== LOGIN PAGE ===================== */
function LoginPage({onLogin,brandLogo,onBrandUpload}){
  ...
  const googleLogin=async()=>{
    ...
    try {
      ...
      const text = await res.text();
      const sheetUsers = parseCsv(text);
      // Sign in as first Admin user in sheet
      const admin = sheetUsers.find(a => a.role === "Admin") || sheetUsers[0];
      if (admin) { onLogin(admin); return; }
    } catch(e) {}
    onLogin(AUTH_ACCOUNTS[0]);
  };
  const handleBrandUpload=e=>{
    const file=e.target.files?.[0];
    if(!file) return;
    const reader=new FileReader();
    reader.onload=ev=>onBrandUpload(ev.target.result);
  ...
  useEffect(()=>{
    ...
    actions.forEach(a=>{
      (Array.isArray(a.messages)?a.messages:[]).forEach(m=>{
        if(m.text&&m.text.includes("@"+user.name)&&m.author!==user.name){
          newNotifs.push({id:"msg_"+a.id+"_"+m.ts,type:"mention",icon:"💬",
    ...
    audit.forEach(e=>{
      ...
      if(e.target===user.role||e.target==="All"){
        newNotifs.push({id:"esc_"+e.id,type:"escalation",icon:"🚨",
    ...
    actions.filter(a=>a.responsible===user.name&&a.revisions>0).forEach(a=>{
      ...
      if(lastRev){
        newNotifs.push({id:"rev_"+a.id,type:"revision",icon:"📅",
    ...
    actions.filter(a=>a.responsible===user.name&&isOverdue(a)).forEach(a=>{
      newNotifs.push({id:"over_"+a.id,type:"overdue",icon:"⚠",
...
function Shell({children,page,setPage,user,onLogout,onQuickAdd,brandLogo,pendingCount,auditCount,activeMtg,onResumeActiveMtg,mtgRunning,mtgElapsed,notifications,onMarkAllRead,unreadCount,users,actions,onShowSupport,onShowProfile,onShowAdminNotifs}){
      ...
      <aside style={{width:228,background:T.navy,color:"#fff",display:"flex",flexDirection:"column",flexShrink:0,position:"sticky",top:0,height:"100vh"}}>
        <div style={{padding:"20px 18px 16px",borderBottom:"1px solid rgba(255,255,255,.1)"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            {brandLogo
              ? <img src={brandLogo} alt="Brand" style={{width:36,height:36,objectFit:"contain",borderRadius:8,background:"#fff",padding:2}}/>
            ...
            {!exitConfirm
              ...
              :<div className="card" style={{padding:16,border:`1.5px solid ${T.red}40`,background:T.redL,textAlign:"left",animation:"fadeIn .2s ease"}}>
                ...
                <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
                  ...
                  <button className="btn btn-red btn-sm" onClick={()=>{stopSTT();onCloseMeeting();}}>Yes, Exit & Discard</button>
                </div>
              </div>
            }
          </div>
...
function StagingArea({staged,mtg,plants,depts,users,txLines,onCommit,onCloseMeeting,onBack,elapsedSecs}){
  ...
  const commitAll=()=>{
    const all=draft.filter(r=>r.text?.trim()).map((r,i)=>{
      ...
      return{...r,sn:nextSN(Array(i)),id:Date.now()+i,dateOfAction:todayStr(),revisions:0,revisionHistory:[],created:todayStr(),closedOn:null,pendingConfirmation:false,
...
<div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
        ...
        <KPICard icon="🚨" value={audit.length} label="Escalated Actions"  sub="View all →"            color={T.red}    alert={audit.length>0} onClick={()=>setDrill("escalated")}/>
...
<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
        ...
        <div className="card" style={{padding:20}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              <span style={{background:T.redL,color:T.red,padding:"3px 9px",borderRadius:20,fontSize:11,fontWeight:700}}>{audit.length} logged</span>
      ...
      {drill==="criticalOverdue"&&(()=>{
        ...
        return(
          <>
            ...
            <div className="side-panel" style={{width:480,padding:0}}>
...
export default function App(){
  ...
  if(!user) return(<><style>{CSS}</style><LoginPage onLogin={acc=>{setUser(acc);setPage(0);}} brandLogo={brandLogo} onBrandUpload={setBrandLogo}/></>);

  return(
    <>
      <style>{CSS}</style>
      <Shell page={page} setPage={setPage} user={user} onLogout={()=>{setUser(null);setPage(0);clearMeetingState();}} onQuickAdd={()=>setShowQuickAdd(true)} brandLogo={brandLogo} pendingCount={pendingForMe} auditCount={audit.length} activeMtg={globalActiveMtg} onResumeActiveMtg={()=>setPage(1)} mtgRunning={mtgRunning} mtgElapsed={mtgElapsed} notifications={notifs} unreadCount={unreadNotifs} onMarkAllRead={markAllRead} users={users} actions={actions} onShowSupport={()=>setShowSupport(true)} onShowProfile={()=>setShowProfile(true)} onShowAdminNotifs={()=>setShowAdminNotifs(true)}>
